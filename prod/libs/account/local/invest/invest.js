const dailyProfit = document.querySelector('#invest_amount_daily');
const totalProfit = document.querySelector('#invest_amount_total');
const cryptoType = document.querySelector('#invest_amount_crypto');
/*
let fanInputMaskOptions = {
	radix: '.',  // fractional delimiter
	mask: Number
};
IMask(dailyProfit, fanInputMaskOptions);
IMask(totalProfit, fanInputMaskOptions);
IMask(cryptoType, fanInputMaskOptions); */
if(totalProfit) {
totalProfit.addEventListener('input', function(e){
	if(e.target.value > 45000) {
		e.target.value = 45000
	}
});
}
...
$(document).on ("input", "#invest_amount_fiat", function () {
	...
	if (! isNaN (amountFiat)) {
		...
		amountDaily = (amountFiat/10).toFixed(2);
		amountTotal = amountFiat * 1.5;
...
$(document).on ("input", "#invest_amount_daily", function () {
	...
	if (! isNaN (amountDaily)) {
		amountFiat = Math.round (amountDaily / 10 * 100 * Math.pow (10, currency.decimal.fiat)) / Math.pow (10, currency.decimal.fiat);
...
$(document).on ("input", "#invest_amount_total", function () {
	...
	if (! isNaN (amountTotal)) {
		amountDaily = (amountTotal/15).toFixed(2);
		amountFiat = (amountTotal/15 * 10).toFixed(2);
